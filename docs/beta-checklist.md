...
12. Open the `<id>.kicad_pro` in KiCad 9.0 or newer. KiCad opens it cleanly with no earlier-version conversion prompt. The STM32F405RG and Flash (W25Q128JV) symbols render from the user's installed KiCad stock libraries (#38). RP2040, IMU (ICM-42688-P), barometer (BMP388), USB-UART bridge (CH340K), and Tag-Connect debug header render from vendored bodies inlined in `(lib_symbols ...)` per [ADR 0002](adr/0002-symbol-vendoring.md) (PRs #46 / #49 / #51 / #52 / #53). Attribution ships in `KIFORGE-LICENSE.txt` and [catalog/symbols/LICENSE.md](../catalog/symbols/LICENSE.md).
...
- [x] Symbol vendoring Phase 2 (#49 Tag-Connect, #51 BMP388, #52 CH340K, #53 ICM-42688-P) — all four catalog parts now ship a vendored `.kicad_sym` body, wired via `kicadLibId` + `vendoredSymbolPath` in their catalog JSON and attributed in [catalog/symbols/LICENSE.md](../catalog/symbols/LICENSE.md).
- [x] Symbol vendoring Phase 3 — exported `.kicad_sch` is prepended with a `;; Generated by KiForge…` license attribution comment per [ADR 0002](adr/0002-symbol-vendoring.md).
