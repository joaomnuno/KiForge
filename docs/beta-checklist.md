...
12. Open the `<id>.kicad_pro` in KiCad 7 or newer. KiCad opens it cleanly. The controller (STM32F405RG) and Flash (W25Q128JV) symbols render from the user's installed KiCad stock libraries (#38). The IMU (ICM-42688-P), barometer (BMP388), USB-UART bridge (CH340K), and Tag-Connect debug header render from vendored bodies inlined in `(lib_symbols ...)` per [ADR 0002](adr/0002-symbol-vendoring.md) (PRs #46 / #49 / #51 / #52 / #53). The schematic's first two lines are a `;; Generated by KiForge…` license attribution comment.
